...
export class PluginManager implements OnApplicationBootstrap {
  ...
  private async getDashboardContent(guildId: string, pluginId: string): Promise<GuildPluginDetail['dashboardContent']> {
    ...
    return {
      mode: 'schema',
      schema: createGeneratedDashboardSchema(manifest),
      bundleUrl: null,
      assetsBaseUrl: `/api/v1/guilds/${guildId}/plugins/${pluginId}/assets`,
...
function createGeneratedDashboardSchema(manifest: PluginManifest): PluginDashboardSchemaDocument {
  const tabs = manifest.dashboard?.tabs?.length ? manifest.dashboard.tabs : getGeneratedTabs(manifest);
  return {
    version: 1,
    contentMode: 'schema',
    defaults: {},
    ...
    tabs: tabs.map((tabId) => ({
      ...
      sections: [
        {
          id: `${tabId}.generated`,
          title: `${toTitle(tabId)} management`,
          description: getGeneratedSectionDescription(tabId, manifest),
          fields: [],
          actions: [],
        },
      ],
...
function getGeneratedTabs(manifest: PluginManifest): string[] {
  const tabs = ['overview'];
  if (manifest.capabilities.dashboard || manifest.capabilities.database || manifest.capabilities.templates) {
    tabs.push('settings');
  }
...
function getGeneratedSectionDescription(tabId: string, manifest: PluginManifest): string {
  if (tabId === 'overview') {
    return manifest.description;
  }
  if (tabId === 'commands') {
    return 'Commands registered by this plugin are managed through Nexura Core command APIs.';
  }
  if (tabId === 'logs') {
    return 'Runtime records and audits for this plugin are managed through Nexura Core logs.';
  }
  return 'This plugin did not provide configurable fields for this tab. Upload a package with dashboard.schema.json to expose custom settings.';
}
